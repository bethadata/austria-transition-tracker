"""ARCHIVED 2026-08-25 -- the Plotly figure builders from the Jekyll era.

Kept for reference only. Nothing imports this module and nothing should: it is
here so that the *layout* decisions taken over two years of the old dashboard --
which series is drawn in ink, how a negative stack is split, where the source
annotation sits, the 0.6 fill opacity -- are still readable next to the Vue
components that now own them (`src/components/TimeSeriesChart.vue`).

Why it stopped being live code: the 2026-08-24 migration deleted the HTML writer, so
`save_figure()` only handed `data_plot` to the manifest and the frontend rendered
from JSON. These two functions kept building a complete `go.Figure` -- traces,
colours, hovertemplates, layout, annotations -- and then threw it away. Roughly
600 lines, and every `colors=`, `plotmax_fac=` and `legend_inside=` argument at
85 call sites, fed nothing but this.

`plot_single_go` handled line / area / area_neg / bar / area_button;
`plot_with_toggle` built all three views of a toggle chart at once and switched
them with a Plotly updatemenu. Both are superseded by one manifest-driven
component.
"""
