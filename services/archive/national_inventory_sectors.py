# -*- coding: utf-8 -*-
"""ARCHIVED 2026-08-25 -- the National Inventory Report sub-sector reader.

Superseded by `sources/eea.py`, which does the same job from the EEA
greenhouse-gas data viewer export. Both built a sector's sub-sector breakdown,
scaled the difference against the Umweltbundesamt total into an "Other" bucket
and extrapolated the newest year; `plot_emissions_sectors` switched to the
EEA one and this was left behind, imported by seven modules and called by
none.

Kept because it is not a straight duplicate: it reads the *NIR* rather than the
EEA export -- 34 hand-transcribed `.txt` tables under
`data_raw/national_inventory_report/` -- and it carries source-specific
knowledge the EEA route does not need and could not reconstruct:

  * the F-gas conversions, including the GWPs used for aluminium PFC emissions
    weighted by the CF4/C2F6 split per tonne of aluminium
  * the 2021 patch for ozone-depleting substitutes, which the 2023 inventory
    reported wrongly and which is scaled from the 2020 ratio
  * splitting waste-to-energy out of public electricity and heat by fuel share,
    which is precisely the TODO still open in `sources/eea.py`

`get_land_uses()` -- the land-use areas for the LULUCF page -- is still live and
stayed behind in `sources/national_inventory.py`.
"""
...
import numpy as np 
# from ..sources.umweltbundesamt import sectoral_emissions  (archived: do not wire up)
